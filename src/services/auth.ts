...
export const getTerminals = async (): Promise<ApiResponse<User>> => {
  try {
    const response = await get(
      `${baseUrl.replace(/\/$/, '')}/companies/${import.meta.env.VITE_COMPANY_ID}/terminals`
    );
    if (!response) {
      return {
        error: {
...
export const getLookup = async (
  ...
  try {
    const response = await get(
      `${baseUrl.replace(/\/$/, '')}/lookup?companyId=${import.meta.env.VITE_COMPANY_ID}&terminalId=${terminalId}`
    );
    if (!response) {
      return {
        error: {
...
export const forgotPassword = async (
  ...
  try {
    const response = await post<SignInResponse>(
      `${baseUrl.replace(/\/$/, '')}/user/forgot-password`,
      {
        username,
      },
      true
