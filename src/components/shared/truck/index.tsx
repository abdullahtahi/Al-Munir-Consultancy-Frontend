...
import { useTruckDriverByAttributes, useTruckNumbers } from 'src/api/hooks/useTruckDriverByAttributes';
...
const TruckSelector: React.FC<Props> = ({
    ...
    const { data: trucksNumbersResp = { rows: [] }, isLoading: isLoadingNumbers } =
        useTruckNumbers(
            resolvedCompanyId,
            resolvedTerminalId,
            {
    ...
    const isOptionEqualToValue = (option: TruckType, value: TruckType) => option?.id === value?.id;
