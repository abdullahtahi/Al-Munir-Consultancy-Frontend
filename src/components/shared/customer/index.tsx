...
const CustomerSelector: React.FC<Props> = ({
    ...
    const { data: customers = [], isLoading } = resolvedTerminalId && resolvedCompanyId
        ? useCustomersByAttributes(
            resolvedCompanyId,
            resolvedTerminalId,
            {
                limit: constants.all,
    ...
    const isOptionEqualToValue = (option: CustomerType, value: CustomerType) => option.id === value?.id;
    ...
    if (formik && !isSearchFilter) {
        ...
        return (
            <Autocomplete
                ...
                disabled={disabled}
                value={
                    customers.find((cust: CustomerType) => cust.id === +formik.values[name]) || null
                }
                renderInput={(params) => (
                    <MuiTextField
                        {...params}
