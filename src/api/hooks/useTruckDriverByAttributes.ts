...
import { getAllDrivers, getAllTrucks, getDriverIds, getTruckDriversByAttributes, getTruckNumbers } from '../truck-driver';
...
export const useTruckDriverByAttributes = (
    ...
    const shouldFetch = enabled && companyId && terminalId && transporterId && hasValidSearch;

    const fetcherKey = shouldFetch
        ? ['truck-driver-by-attributes', companyId, terminalId, transporterId, options]
        : null;

    const { data, error, isLoading, mutate } = useSWR(
        fetcherKey,
        () => getTruckDriversByAttributes(companyId, terminalId, transporterId, options),
        {
...
export const useAllTrucks = (
    ...
    const fetcherKey = shouldFetch ? ['all-trucks', companyId, terminalId, options] : null;
...
export const useTruckNumbers = (
    ...
    const fetcherKey = shouldFetch ? ['truck-numbers', companyId, terminalId, options] : null;
...
export const useAllDrivers = (
    ...
    const shouldFetch = !!(companyId && terminalId);
    const fetcherKey = shouldFetch ? ['all-drivers', companyId, terminalId, options] : null;
...
export const useDriverIds = (
    ...
    const fetcherKey = shouldFetch ? ['driver-ids', companyId, terminalId, options] : null;
